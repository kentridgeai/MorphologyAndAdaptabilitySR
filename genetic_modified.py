...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if isinstance(self, TransformerMixin):
            ...
            print(gen)
            evaluation = np.array([gp.execute(X,y) for gp in
...
class SymbolicRegressor(BaseSymbolic, RegressorMixin):
    ...
    def predict(self, X, y, X_test):
        ...
        y = self._program.execute(X, y, np.array(X_test))
...
class SymbolicClassifier(BaseSymbolic, ClassifierMixin):
    ...
    def predict_proba(self, X,y_train):
        ...
        scores = self._program.execute(X,y_train)
    ...
    def predict(self, X,y_train):
        ...
        """
        proba = self.predict_proba(X,y_train)
...
class SymbolicTransformer(BaseSymbolic, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X_new = np.array([gp.execute(X,y) for gp in self._best_programs]).T
